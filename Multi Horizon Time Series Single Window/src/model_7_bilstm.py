...
PURPOSE: Reads sequence forward AND backward.
         Usually highest F1 of single-head baselines.
         YOUR model8 must beat this.
         Still single output — cannot produce curve.

RUN:
    python model7_bilstm.py
"""
...
class SimpleBiLSTM(nn.Module):
    """
    Bidirectional LSTM.
    Reads sequence week1→week8 AND week8→week1.
    Combines both directions for richer representation.
    Hidden size doubles because of two directions.
    Still single output — cannot produce curve.

    This is the STRONGEST single-head baseline.
    YOUR model8 must outperform this.
    """
    ...
    def __init__(self,
        ...
        # Input to classifier is hidden*2
        # because forward + backward outputs concatenated
    ...
    def forward(self, x):
        ...
        return self.classifier(bilstm_out[:, -1, :])
...
print_metrics("BiLSTM", f1, auc, pre, rec,
              can_curve=True)
print(f"Best val F1: {best_val_f1:.4f} at epoch {best_epoch}")
print(f"\nNOTE: This is the strongest baseline.")
print(f"YOUR model7 must beat F1={f1:.4f}")