...
const RISK_COLOR = { Low: '#16a34a', Medium: '#b45309', High: '#f97316', Critical: '#ef4444', Unknown: '#475569' }
const RISK_SCORE = { Low: 1, Medium: 3, High: 7, Critical: 10, Unknown: 0 }
const RISK_LEFT  = { Low: 14, Medium: 12, High: 8, Critical: 4, Unknown: 0 }

const ADVISOR_MAP = {
  Normal:               { text: '✅ Student appears emotionally stable. Continue routine monitoring.',                                                                                          color: '#0891b2', bg: '#22d3ee0d', border: '#22d3ee30' },
  Stress:               { text: '⚠️ Student showing stress indicators. Recommend check-in with advisor.',                                                                                     color: '#b45309', bg: '#fbbf240d', border: '#fbbf2430' },
  Anxiety:              { text: '⚠️ Student showing anxiety indicators. Recommend counseling session.',                                                                                       color: '#b45309', bg: '#fbbf240d', border: '#fbbf2430' },
  Depression:           { text: '🔴 HIGH RISK: Student showing depression indicators. Immediate counseling recommended. Key trigger words are highlighted in the attention heatmap above.',    color: '#dc2626', bg: '#f871710d', border: '#f8717130' },
  ...
  Bipolar:              { text: '🔴 HIGH RISK: Student showing bipolar indicators. Recommend psychiatric evaluation.',                                                                         color: '#dc2626', bg: '#f871710d', border: '#f8717130' },
  'Personality disorder': { text: '🔴 HIGH RISK: Student showing personality disorder indicators. Recommend professional evaluation.',                                                         color: '#dc2626', bg: '#f871710d', border: '#f8717130' },
}
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, padding: '16px 20px' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }}>
        ...
        {yTicks.map(v => (
          <g key={v}>
            <line x1={padL} y1={yOf(v)} x2={padL+chartW} y2={yOf(v)} stroke="#cbd5e1" strokeWidth="1" strokeDasharray="4 4" />
            <text x={padL-6} y={yOf(v)+4} textAnchor="end" fontSize="9" fill="#64748b">{v}</text>
          </g>
        ))}
        ...
        {WEEK_NUMS.map(w => (
          <text key={w} x={xOf(w)} y={padT+chartH+16} textAnchor="middle" fontSize="9" fill="#64748b">Wk {w}</text>
        ))}
        <path d={areaPath} fill="url(#chartAreaGrad)" />
        <path d={linePath} fill="none" stroke="url(#chartLineGrad)" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="7" fill="#ffffff" stroke={p.color} strokeWidth="2.5" />
        ...
        <text x={padL-2} y={padT-10} fontSize="9" fill="#64748b" textAnchor="middle">Score</text>
      </svg>
    </div>
  )
}
...
function AddStudentForm({ onSaved }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '24px' }}>
      ...
      {WEEK_NUMS.map(w => (
        <div key={w} style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#64748b', letterSpacing: 1 }}>
              WEEK {w} <span style={{ color: '#94a3b8', fontWeight: 400 }}>(optional)</span>
            </div>
            {texts[w].length === 1
              ? (
                <button
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = '#4f46e5'; e.currentTarget.style.color = '#4338ca' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 10, color: '#4f46e5', fontWeight: 600 }}>2 submissions</span>
                  <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {texts[w].map((val, idx) => (
              <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {texts[w].length > 1 && (
                  <div style={{ fontSize: 9, fontWeight: 700, color: '#64748b', letterSpacing: 1 }}>
                    SUBMISSION {idx + 1}
                  </div>
                )}
                <textarea
        ...
        : (
          <button
            ...
            style={{
              ...
              border: `1px solid ${canSave ? '#4f46e5' : '#cbd5e1'}`,
              color: canSave ? '#4338ca' : '#64748b',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          return (
            <div key={w.week} style={{ display: 'flex', gap: 0 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 36, flexShrink: 0 }}>
                <div style={{ width: 12, height: 12, borderRadius: '50%', background: hasData ? col : '#cbd5e1', boxShadow: hasData ? `0 0 6px ${col}80` : 'none', marginTop: 16, flexShrink: 0 }} />
                {i < chartData.length - 1 && <div style={{ width: 2, flex: 1, background: '#e2e8f0', minHeight: 16 }} />}
              </div>
              <div style={{ flex: 1, background: '#ffffff', border: `1px solid ${hasData ? `${col}30` : '#e2e8f0'}`, borderRadius: 10, padding: '12px 14px', marginLeft: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: hasData ? 8 : 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: hasData ? '#475569' : '#94a3b8', letterSpacing: 1 }}>WEEK {w.week}</div>
                  {hasData
                ...
                {hasData && (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {[['PREDICTION', w.prediction], ['CONFIDENCE', `${w.confidence}%`], ['RISK SCORE', `${w.riskScore}/10`], ['WEEKS TO INTERVENE', `${w.weeksLeft} wks`]].map(([label, value]) => (
                      <div key={label} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 6, padding: '8px 12px', flex: 1, minWidth: 90 }}>
                        <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: '#64748b', marginBottom: 4 }}>{label}</div>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #cbd5e1' }}>
          ...
          <thead>
            <tr style={{ background: '#f1f5f9' }}>
              {['Week', 'Prediction', 'Confidence', 'Risk Score', 'F1 Score', 'Weeks Left'].map(h => (
                <th key={h} style={{ padding: '10px 16px', fontSize: 10, fontWeight: 700, color: '#64748b', letterSpacing: 1, textAlign: 'left' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {chartData.map((w, i) => {
              ...
              return (
                <tr key={w.week} style={{ borderTop: '1px solid #e2e8f0', background: i % 2 === 0 ? 'transparent' : '#0f172a08' }}>
                  <td style={{ padding: '10px 16px', fontSize: 13, color: '#475569', fontWeight: 600 }}>Week {w.week}</td>
                  ...
                  <td style={{ padding: '10px 16px', fontSize: 13, color: w.riskScore > 0 ? '#6d28d9' : '#64748b', fontFamily: 'monospace', fontWeight: 600 }}>{f1}</td>
                  <td style={{ padding: '10px 16px', fontSize: 13, color: '#475569' }}>{w.weeksLeft > 0 ? w.weeksLeft : '—'}</td>
                </tr>
              )
            })}
          </tbody>
...
function WeekAnalysisCard({ weekNum, weekData, isLatest }) {
  ...
  const col = RISK_COLOR[result.risk_level] || '#475569'

  return (
    <div style={{
      background: '#ffffff',
      border: `1px solid ${open ? `${col}50` : '#cbd5e1'}`,
      borderRadius: 10,
      overflow: 'hidden',
      ...
      >
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b', letterSpacing: 1, minWidth: 52 }}>
          WEEK {weekNum}
          {isLatest && <span style={{ marginLeft: 6, fontSize: 9, color: '#4f46e5', background: '#6366f115', border: '1px solid #6366f130', borderRadius: 4, padding: '1px 5px', letterSpacing: 0.5 }}>LATEST</span>}
        </div>
        ...
        {/* Text preview */}
        <div style={{ flex: 1, fontSize: 12, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontStyle: 'italic' }}>
          "{text}"
        </div>

        {/* Chevron */}
        <div style={{ fontSize: 12, color: '#64748b', transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s', flexShrink: 0 }}>▼</div>
      </div>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${col}30`, padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Full text */}
          <div style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '12px 14px' }}>
            <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: '#64748b', marginBottom: 6 }}>STUDENT TEXT</div>
            <div style={{ fontSize: 13, color: '#475569', lineHeight: 1.7, fontStyle: 'italic' }}>"{text}"</div>
          </div>
...
function AnalysisTab({ student, latest, worst }) {
  if (!latest) {
    return <div style={{ fontSize: 13, color: '#64748b', padding: 20 }}>No analysis data available for this student.</div>
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* Overall summary row */}
      <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, padding: '14px 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={label} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '10px 14px', flex: 1, minWidth: 110 }}>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: '#64748b', marginBottom: 4 }}>{label}</div>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, padding: '16px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, letterSpacing: 1 }}>IT22196392 — R26-IT-059</div>
        </div>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#4f46e5', marginBottom: 12 }}>BERT Fine-tuned + RoBERTa Ensemble</div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[['ACCURACY', '82.32%', 'Test set'], ['F1 SCORE', '82.32%', 'Weighted avg'], ['DATASET', '51,055', 'Training samples'], ['CLASSES', '7', 'Mental health categories']].map(([label, value, sub]) => (
            <div key={label} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '12px 16px', flex: 1, minWidth: 100 }}>
              ...
              <div style={{ fontSize: 10, color: '#64748b', marginTop: 3 }}>{sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* CSV export */}
      <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 10, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: '#64748b', marginBottom: 4 }}>EXPORT DATA</div>
          <div style={{ fontSize: 11, color: '#64748b' }}>Export for Meta-Integration Layer</div>
        </div>
        <button
          onClick={() => {
            const rows = [
...
function WelcomeState({ onAddNew }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, height: '100%', minHeight: 400, padding: 40 }}>
      ...
      <div style={{ fontSize: 14, color: '#64748b', textAlign: 'center', maxWidth: 340, lineHeight: 1.7 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '24px', flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>ID: {student.id}</div>
        </div>
      ...
      {isCritical && (
        <div style={{ background: '#ff000012', border: '1px solid #ff000040', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#dc2626', marginTop: 2 }}>This student's text indicates critical distress. Contact a mental health professional immediately.</div>
          </div>
        </div>
      )}
