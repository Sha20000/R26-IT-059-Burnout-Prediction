...
# Save real, computed cross-year validation results to CSV so the API
# and dashboard can read live numbers instead of a hard-coded dict.
cross_year_df = pd.DataFrame(results_rows)
cross_year_df.to_csv("../data/processed/cross_year_validation.csv", index=False)
print("\nSaved: cross_year_validation.csv")
print(cross_year_df)
