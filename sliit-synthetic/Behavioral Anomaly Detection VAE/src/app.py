...
# Load SLIIT synthetic-data results (Karunarathne D C — SLIIT BurnoutDetector)
@st.cache_data
def load_sliit_data():
    base = "../../SLIIT BurnoutDetector/results/metrics"
    sliit_anomaly = pd.read_csv(f"{base}/sliit_anomaly_scores_final.csv")
    sliit_features = pd.read_csv(f"{base}/sliit_features_final.csv")
    sliit_expected = pd.read_csv(f"{base}/sliit_expected_behavior.csv")
    return sliit_anomaly, sliit_features, sliit_expected

sliit_anomaly_df, sliit_features_df, sliit_expected_df = load_sliit_data()
...
# ─────────────────────────────────────────
# PAGE 5 — SLIIT SYNTHETIC DATA
# ─────────────────────────────────────────
elif page == "🎓 SLIIT Synthetic Data":
    st.title("🎓 SLIIT Synthetic Dataset — Behavioral Anomaly Results")
    st.markdown("### SLIIT BurnoutDetector Module (Synthetic Cohort)")
    st.markdown("**R26-IT-059 | IT22215710 | Karunarathne D C**")
    st.markdown("---")

    total_sliit_students = sliit_anomaly_df['student_id'].nunique()
    at_risk_students = sliit_anomaly_df.groupby('student_id')['at_risk'].first()
    n_at_risk = int(at_risk_students.sum())
    avg_anomaly = sliit_anomaly_df['anomaly_score'].mean()
    avg_compliance = sliit_anomaly_df['curriculum_compliance'].mean()

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Students (Synthetic Cohort)", f"{total_sliit_students:,}")
    col2.metric(
        "At-Risk Students",
        f"{n_at_risk:,}",
        delta=f"{n_at_risk/total_sliit_students*100:.1f}% of total"
    )
    col3.metric("Avg Anomaly Score", f"{avg_anomaly:.4f}")
    col4.metric("Avg Curriculum Compliance", f"{avg_compliance:.3f}")
    ...
    with col1:
        ...
        fig = px.histogram(
            ...
            color='at_risk',
            color_discrete_map={0: 'steelblue', 1: 'red'},
            labels={'anomaly_score': 'Anomaly Score', 'at_risk': 'At Risk'},
            title='Anomaly Score Distribution by At-Risk Status',
            barmode='overlay',
            opacity=0.7
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        st.subheader("🧬 Student Type Breakdown")
        type_counts = sliit_anomaly_df.groupby(
            'student_id'
        )['student_type'].first().value_counts().reset_index()
        type_counts.columns = ['student_type', 'count']
        fig2 = px.bar(
            type_counts,
            x='student_type',
            y='count',
            color='student_type',
            title='Students by Behavioral Profile',
            labels={'student_type': 'Student Type', 'count': 'Students'}
        )
        st.plotly_chart(fig2, use_container_width=True)
    ...
    with col1:
        st.subheader("📈 Weekly Combined Score Trend")
        weekly = sliit_anomaly_df.groupby('week')['combined_score'].mean().reset_index()
        fig3 = px.line(
            weekly,
            x='week',
            y='combined_score',
            title='Average Combined Risk Score per Week',
            markers=True
        )
        st.plotly_chart(fig3, use_container_width=True)

    with col2:
        st.subheader("📚 Avg Compliance by Module")
        module_compliance = sliit_anomaly_df.groupby(
            'module_code'
        )['curriculum_compliance'].mean().reset_index()
        fig4 = px.bar(
            module_compliance,
            x='module_code',
            y='curriculum_compliance',
            title='Average Curriculum Compliance by Module',
            labels={'module_code': 'Module', 'curriculum_compliance': 'Avg Compliance'}
        )
    ...
    sliit_student_ids = sorted(sliit_anomaly_df['student_id'].unique())
    selected_sliit_student = st.selectbox(
        "Select Student ID", sliit_student_ids, key="sliit_student_select"
    )

    student_row = sliit_anomaly_df[
        sliit_anomaly_df['student_id'] == selected_sliit_student
    ].sort_values('week')

    if len(student_row) > 0:
        name = student_row['student_name'].iloc[0]
        stype = student_row['student_type'].iloc[0]
        risk = student_row['at_risk'].iloc[0]

        c1, c2, c3 = st.columns(3)
        c1.metric("Student", name)
        c2.metric("Behavioral Type", stype)
        c3.metric("Status", "🔴 AT RISK" if risk == 1 else "🟢 NORMAL")

        fig5 = go.Figure()
        fig5.add_trace(go.Scatter(
            x=student_row['week'], y=student_row['anomaly_score'],
            mode='lines+markers', name='Anomaly Score',
            line=dict(color='steelblue', width=2)
        ))
        fig5.add_trace(go.Scatter(
            x=student_row['week'], y=student_row['combined_score'],
            mode='lines+markers', name='Combined Score',
            line=dict(color='orange', width=2)
        ))
        fig5.update_layout(
            xaxis_title='Week', yaxis_title='Score',
            title=f'{name} ({selected_sliit_student}) — Risk Timeline'
        )
        st.plotly_chart(fig5, use_container_width=True)

    st.markdown("---")

    st.subheader("🏆 Model Evaluation (SLIIT Synthetic Cohort)")
    fig_dir = "../../SLIIT BurnoutDetector/results/figures"
    fig_cols = st.columns(2)
    sliit_figures = [
        ("sliit_roc_curve.png", "ROC Curve — SLIIT Synthetic Cohort"),
        ("sliit_module_auc.png", "ROC-AUC by Module"),
        ("sliit_temporal_lag_comparison.png", "Temporal Lag Analysis — SLIIT vs. OULAD"),
        ("sliit_shap_importance.png", "SHAP Feature Importance — SLIIT Model"),
        ("sliit_vae_training_loss.png", "VAE Training Loss — SLIIT Model"),
        ("sliit_behavioral_patterns.png", "Behavioral Patterns by Student Type"),
    ]
    for i, (fname, caption) in enumerate(sliit_figures):
        fpath = f"{fig_dir}/{fname}"
        if os.path.exists(fpath):
            with fig_cols[i % 2]:
                st.image(Image.open(fpath), caption=caption)

    st.caption(
        "Synthetic SLIIT cohort (500 students, 5 modules, weeks 4-15) generated to "
        "validate the behavioral anomaly pipeline on multi-module concurrent enrolment, "
        "a real-world scenario not present in the single-module OULAD dataset."
    )
